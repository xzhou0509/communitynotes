...
def multimodal_search(api_key, tweet, page_num):
    ...
    for image_url in tweet['tweet_images']:
        ...
        if queries in ['NONE', '\"NONE\"']: # No queries, search using images
            ...
            os.makedirs('eval/data/web_search/reverse_image/', exist_ok=True)
            ...
            for x, query_x in enumerate(queries):
                ...
                # print(qx)
                os.makedirs('data/web_search_multimodal/', exist_ok=True)
...
def query_search(api_key, search_engine_id, tweet, domain_priority):
    ...
    for x, query_x in enumerate(queries):
        ...
        os.makedirs('data/web_search_text/', exist_ok=True)
        if not os.path.exists(save_file):
            with open(save_file, 'w') as f:
...
def correction_generation(tweet, list_of_evidences, list_of_refute_evidences, llm_key):
    if len(list_of_refute_evidences) == 0:
        ...
        #     prompt = f.read()
        return "NOT ENOUGH EVIDENCE TO WRITE A GOOD COMMUNITY NOTE."
    else:
...
def correction_condense(correction, llm_key):
    with open('data/prompt_correction_condense.txt', 'r') as f:
        prompt = f.read()
    prompt = prompt.replace('[CORRECTION]', correction)
    condensed_correction = gpt(llm_key, prompt)
    print("\t(sent 1 request to llm)")
    return condensed_correction

def tags_identify(correction, llm_key):
    with open('data/prompt_tag_identification.txt', 'r') as f:
        prompt = f.read()
...
def muse(data_json, tweet_modality="unimodal"):
    ...
    # svc = Service(log_output="geckodriver.log")  # writes detailed driver logs
